  ...
  try {
    ...
    if (!user) {
      if (cookieRawLink || cookieCode) {
        ...
        if (inm && inm === etag) return new NextResponse(null, { status: 304, headers: { 'ETag': etag, 'Cache-Control': 'private, no-store' } })
        const res = NextResponse.json({ ok: true, status: code ? 'valid' : 'not_found', rawLink: cookieRawLink || null, code: code || null, expiresAt: null, totalInvites }, { headers: { 'Cache-Control': 'private, no-store', 'ETag': etag } })
      ...
      const inm = req.headers.get('if-none-match')
      if (inm && inm === etag) return new NextResponse(null, { status: 304, headers: { 'ETag': etag, 'Cache-Control': 'private, no-store' } })
      const res = NextResponse.json({ ok: true, status: 'not_found', totalInvites }, { headers: { 'Cache-Control': 'private, no-store', 'ETag': etag } })
      try { res.cookies.set('wspace_ref_total', String(totalInvites), { httpOnly: false, sameSite: 'lax', path: '/', secure: process.env.NODE_ENV === 'production' }) } catch {}
      return res
    }
    ...
    const res = NextResponse.json(payload, { headers: { 'Cache-Control': 'private, no-store', 'ETag': etag } })
    try { res.cookies.set('wspace_ref_total', String(totalInvites), { httpOnly: false, sameSite: 'lax', path: '/', secure: process.env.NODE_ENV === 'production' }) } catch {}
    return res
  } catch {
